...
* To enable eBPF mode with the automatic configuration, set the `spec.calicoNetwork.bpfNetworkBootstrap` and `spec.calicoNetwork.kubeProxyManagement` parameters in the operator's `Installation`
  ...
  The operator rolls out the change with a rolling update (non-disruptive) and then swiftly transitions all nodes t  o eBPF mode.
  However, it's inevitable that some nodes will enter eBPF mode before others.
  This can disrupt the flow of traffic through node ports.
...
- VLAN-based traffic without hardware offloading.
  :::tip
  You can use a VLAN device to connect a node to the cluster by using the `bpfDataIfacePattern` Felix configuration variable if the underlying/physical device supports VLAN offloading.
...
In GCP, the source/dest check should also be disabled, which can be done by enabling IP forwarding.
